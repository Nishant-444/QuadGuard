...
> _"Optimizing only the current incident may lead to poor system-wide performance."_
...
4. **Repair Pass for Simultaneous Departures**: If multiple vehicles in the same quadrant are assigned simultaneously in a single minute such that the quadrant would be completely emptied, [`repair_pass`](file:///home/nishant/Code/QuadGuard/dispatcher.py#L62-L81) in [`dispatcher.py`](file:///home/nishant/Code/QuadGuard/dispatcher.py) identifies the batch and drops the assignment with the lowest priority weight, retaining $\ge 1$ vehicle in idle coverage.
